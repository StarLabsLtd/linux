...
#define RTSX_USB_SD_CD_DEBOUNCE_MS	20
...
static int sdmmc_get_cd(struct mmc_host *mmc)
{
	...
	if (host->cd_debounce < RTSX_USB_SD_CD_DEBOUNCE_CNT) {
		host->cd_debounce++;
		mmc_detect_change(mmc, msecs_to_jiffies(RTSX_USB_SD_CD_DEBOUNCE_MS));
		goto no_card_unlock;
	}

	/* Avoid pounding the bus with probes if CD is stuck asserted. */
	if (time_before(jiffies, host->next_insert_check)) {
		mmc_detect_change(mmc, host->next_insert_check - jiffies);
		goto no_card_unlock;
	}
